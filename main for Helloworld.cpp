...
int main() {
	// 初始化字母列表
	std::vector<char> letterList = {'G', 'd', 'k', 'k', 'n', ',', 'V', 'n', 'q', 'k', 'c', '!'};
	std::vector<char> processedLetterList;
	
	// 遍历列表，将每个字母的ASCII码值加1
	for (char i : letterList) {
		if (isalpha(i)) {
			processedLetterList.push_back(i + 1); // ASCII加1
		} else {
			processedLetterList.push_back(i); // 非字母直接添加
		}
	}
	
	// 打印处理后的字母
	for (char c : processedLetterList) {
		std::cout << c; // 输出字符
	}
